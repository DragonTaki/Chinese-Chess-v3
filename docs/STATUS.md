...
- **HalfCross（9×5，三國模式）** — 走法／吃子規則已實作，跟 HalfCenter
  完全同一套機制（作者確認：「跟半盤一樣暗棋隨機洗」）——不是三個互相敵對
  的陣營，而是把一整副標準 32 顆棋子（跟大盤一樣：1 將＋2 士＋2 象＋2
  車＋2 馬＋2 包＋5 兵，紅黑各半）隨機洗牌、蓋牌放到board上，本質上是
  「暗棋，只是盤面形狀跟棋子組成換了」。`Rules.HalfCrossTeamSetup` 裡的
  「陣營1／2／3」只是資料寫法上的分組（方便列出「哪些棋子＋各幾顆」），
  不是遊戲邏輯裡真的有三方陣營在打——每顆棋子最終還是只有紅／黑兩種
  `PlayerSide`，跟 HalfCenter 一樣由 `Piece.CanCaptureInDarkChess` 統一判斷
  吃子合法性（因此 `Piece.cs` 裡原本命名為 `...HalfCenter` 的三個共用方法
  已改名為 `...DarkChess`，兩種盤面共用）。
